...
document.addEventListener('DOMContentLoaded', function () {
  const wasteSubmitForm = document.getElementById('wasteSubmitForm');

  wasteSubmitForm.addEventListener('submit', async function (event) {
    event.preventDefault();

    const wasteSubmitUrl = wasteSubmitForm.getAttribute('data-url');
    ...
    try {
      const formData = new FormData(wasteSubmitForm);
      const formObject = Object.fromEntries(formData.entries());
      const response = await fetch(wasteSubmitUrl, {
        method: 'POST',
        headers: {
